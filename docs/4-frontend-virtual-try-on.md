...
document.getElementById("virtual_try_on_form").onsubmit = async function (e) {
  ...
  formData.append(
    "model_image",
    document.getElementById("model_image").files[0],
  );
  formData.append(
    "clothing_image",
    document.getElementById("clothing_image").files[0],
  );
  ...
  document.getElementById("result_image").src = URL.createObjectURL(
    await response.blob(),
  );
...
3. **Stíluslap hozzáadása**: Ehhez kérjük a beépített GitHub Copilot segítségét. Itt egy promt ehhez:

```text
Te egy webdesigner vagy, aki a frontend stíluslapját készíti el a virtuális próbafülke weboldalhoz. A stíluslap célja, hogy a weboldal modern és felhasználóbarát legyen. A stíluslapnak tartalmaznia kell a következőket:
- A form elemek stílusát, hogy a felhasználók könnyen feltölthessék a képeket.
- A gomb stílusát, hogy kiemelkedjen a weboldalon.
- Az eredmény kép stílusát, hogy a felhasználók könnyen láthassák az eredményt.
- A weboldal általános stílusát, hogy a felhasználók könnyen navigálhassanak a weboldalon.
- A stíluslapnak reszponzívnak kell lennie, hogy a weboldal jól jelenjen meg különböző eszközökön, például mobiltelefonokon és táblagépeken.

Ehhet hozd létre a `style.css` fájlt a `frontend` mappában, és linkeld be az `index.html` fájlban a `<head>` szekcióban.
```

4. **Végső tesztelés**: Nyisd meg a böngészőt, és látogasd meg a `http://localhost:8000` URL-t. Töltsd fel a modell- és ruhadarab képeket, majd kattints a "Próbafülke indítása" gombra. Ellenőrizd, hogy az eredmény kép megjelenik-e az oldalon.
